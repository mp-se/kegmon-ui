    ...
    <div class="container">
        ...
        <div class="row" v-if="state > 1">
            ...
            <div class="row gy-2" v-if="state == 2">
                ...
                <div class="col-md-2">
                    <button @click="back()" type="button" class="btn btn-secondary" :disabled="global.disabled">
                        Back
                    </button>
                </div>
            </div>
        ...
        <div class="row" v-if="state > 2">
            ...
            <div class="col-md-12">
                <p>Place a known weight on the scale and press the factor button. The scale will calculate a factor and
                    complete calibration. Use any known weight.</p>
            </div>

            <div class="col-md-12">
                <BsInputNumber v-model="weight" width="5" label="Calibration weight" min="0" max="25" step=".01"
                    :unit="config.weight_unit" :disabled="global.disabled || state != 3" />
            </div>
        ...
        <div class="row" v-if="state > 3">
            ...
            <div class="row gy-2">
                ...
                <div class="col-md-2">
                    <button @click="begin()" type="button" class="btn btn-secondary" :disabled="global.disabled">
                        Start over
                    </button>
                </div>
            </div>
        ...
        <div class="row p-3">
            ...
            <div :class="getClass()">
                Factor: {{ scaleStatus.factor }}
            </div>
            <div :class="getClass()">
                Raw: {{ scaleStatus.raw }}
            </div>
            <div :class="getClass()">
                Weight: {{ scaleStatus.weightString }}
            </div>
        </div>
...
function getClass() {
    if(config.dark_mode)
        return "col-md-3 p-3 border bg-dark text-white"
...
function saveScaleValues(json) {
    ...
    scaleStatus.value.weightString = new Number(config.weight_unit == "kg" ? scaleStatus.value.weight : weightKgToLbs(scaleStatus.value.weight)).toFixed(3) + " " + config.weight_unit
}
